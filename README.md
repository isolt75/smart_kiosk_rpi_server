...
USB Mic ──┘   └──────────────┘                    │  +YOLOv8 plate detect│
                                                  └─────────┬────────────┘
                                                            │ TransmitJob
                                                            ▼
                                              ┌──────────────────────────┐
                                              │ SenderThread             │ ──▶ Main Server (HTTPS)
                                              │ ThreadPoolExecutor       │     · 원본 이미지
                                              └──────────────────────────┘     · 번호판 crop들
                                                                                · bbox/conf JSON
                                                                                → 메인이 OCR 수행
```
...
- **CaptureThreads** — picamera2 1920×1080 JPEG + sounddevice 16kHz WAV (VAD로
  무음 2초 시 종료)
- **Plate Detector (하이브리드)** — Ultralytics YOLOv8로 번호판 영역만 검출 후
  crop + bbox를 메인 서버에 함께 전송. 모델 미배치 시 자동 폴백(원본만). OCR은
  메인 서버 책임. [SRS §부록 C](docs/SRS.md#부록-c-api-1-v11-확장-필드-번호판-사전-검출)
...
│   ├── inference/
│   │   └── plate_detector.py  # YOLOv8 번호판 영역 검출
│   ├── core/
...
| `ultralytics`, `opencv-python-headless`, `onnxruntime` | YOLOv8 번호판 검출 |
